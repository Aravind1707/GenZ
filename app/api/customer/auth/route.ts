...
export async function POST(request:Request){
  try{
    ...
    const sourceKey=forwarded||request.headers.get('x-real-ip')||'unknown';
  ...
  }catch(e){const message=e instanceof Error?e.message:'';if(message==='Too many OTP requests. Try again later')return NextResponse.json({ok:false,error:message},{status:429});return NextResponse.json({ok:false,error:'Unable to process login request'},{status:400});}
}
