...
export async function POST(req:Request){try{const actor=await auth('members:write');const b=await req.json();
 if(b.type==='NEW'){const result=await createMembership({id:b.id?String(b.id):undefined,name:String(b.name||''),mobile:String(b.mobile||''),tier:b.tier,expiresAt:String(b.expiresAt||''),amount:Number(b.amount||0),method:b.method,staffId:actor.id});await audit(actor.id,'MEMBER_CREATED','MEMBER',result.memberId,{transactionId:result.transactionId,amount:result.amount,method:result.method});return NextResponse.json({ok:true,result},{status:201})}
 const result=await renewMembership({memberId:String(b.memberId||''),newExpiresAt:String(b.newExpiresAt||''),amount:Number(b.amount),method:b.method,staffId:actor.id});await audit(actor.id,'MEMBERSHIP_RENEWED','MEMBER',result.memberId,{transactionId:result.transactionId,amount:result.amount,method:result.method,newExpiresAt:result.newExpiresAt});return NextResponse.json({ok:true,result},{status:201})
