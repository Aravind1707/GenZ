...
async function auth(){const staff=await requireStaff((await cookies()).get(COOKIE)?.value);if(staff.role!=='OWNER'&&staff.role!=='DEVELOPER')throw new Error('STAFF_FORBIDDEN');return staff}
