...
async function auth(){const staff=await requireStaff((await cookies()).get(COOKIE)?.value);if(!['OWNER','DEVELOPER'].includes(String(staff.role)))throw new Error('STAFF_FORBIDDEN');return staff}
