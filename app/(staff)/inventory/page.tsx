...
type Item={itemId:string;name:string;category:string;onHand:number;reserved:number;available:number;reorderLevel:number;unit:string;lowStock:boolean};
...
export default function Inventory(){
 ...
 const load=useCallback(async()=>{setLoading(true);try{const [a,b]=await Promise.all([fetch('/api/inventory',{cache:'no-store'}),fetch('/api/inventory/materials?action=reports',{cache:'no-store'})]);const [d,r]=await Promise.all([a.json(),b.json()]);if(!a.ok)throw Error(d.error||'Unable to load stock');if(!b.ok)throw Error(r.error||'Unable to load inventory reports');setData(d.inventory||[]);setReports(r.reports||{valuation:[],cogs:[]});setError('')}catch(e){setError(e instanceof Error?e.message:'Unable to load inventory')}finally{setLoading(false)}},[]);
 ...
 return <main>
  ...
  <section className="inventoryHero">
   ...
   <div className="card"><span className="label">Items tracked</span><strong>{data.length}</strong><span className="sub">Active materials</span></div>
