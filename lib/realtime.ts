...
export const publish=(type:GenZEventType,data:Record<string,unknown>={})=>{const event:GenZEvent={id:crypto.randomUUID(),type,createdAt:new Date().toISOString(),data};bus.emit('event',event);return event};
