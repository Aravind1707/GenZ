...
export type CustomerPricing={isMember:boolean;gaming:Array<{id:string;label:string;stationType:string;pcTier?:string;specs?:string;imageUrl?:string;regularPrice:number;memberPrice:number;unitLabel:string}>;food:Array<{id:string;name:string;category:string;imageUrl?:string;regularPrice:number;memberPrice:number;available:boolean}>;packages:Array<{id:string;label:string;durationMinutes:number;regularPrice:number;memberPrice:number;extraPlayerSurcharge:number}>};

export async function getCustomerPricing(memberId?:string|null):Promise<CustomerPricing>{
 const memberQuery=memberId?await pool.query<(RowDataPacket & {active:number})[]>('SELECT active FROM members WHERE id=? LIMIT 1',[memberId]):[[] as any];
 const member=memberQuery[0][0]; const activeMember=!!member&&!!member.active;
 ...
 const [packages]=await pool.query<(RowDataPacket & {id:string;label:string;duration_minutes:number;regular_price:number;member_price:number;extra_player_surcharge:number})[]>('SELECT id,label,duration_minutes,regular_price,member_price,extra_player_surcharge FROM gaming_price_packages WHERE active=TRUE ORDER BY station_group,sort_order,duration_minutes');
 return {isMember:activeMember,gaming:gaming.map(r=>({id:r.id,label:r.label,stationType:r.station_type,pcTier:r.pc_tier||undefined,specs:r.specs||undefined,imageUrl:r.image_url||undefined,regularPrice:Number(r.regular_price),memberPrice:Number(r.member_price),unitLabel:r.unit_label})),food:food.map(r=>({id:r.id,name:r.name,category:r.category,imageUrl:r.image_url||undefined,regularPrice:Number(r.non_member_price),memberPrice:Number(r.member_price),available:Boolean(r.available)})),packages:packages.map(r=>({id:String(r.id),label:String(r.label),durationMinutes:Number(r.duration_minutes),regularPrice:Number(r.regular_price),memberPrice:Number(r.member_price),extraPlayerSurcharge:Number(r.extra_player_surcharge)}))};
}
