...
export function normalizeMobile(value:string){
  ...
  if(value.trim().startsWith('+')&&/^\+?[1-9]\d{9,14}$/.test(value.trim())) return value.trim();
  return '';
}

async function sendOtpSms(mobile:string,otp:string){
  const authkey=process.env.GENZ_OTP_MSG91_AUTHKEY;
  const sender=process.env.GENZ_OTP_MSG91_SENDER_ID;
  if(!authkey||!sender){
    if(process.env.NODE_ENV!=='production'&&process.env.GENZ_OTP_DEV_MODE==='true') return;
    throw new Error('OTP provider is not configured');
  }
  const params=new URLSearchParams({authkey,mobile:mobile.replace('+',''),message:`Your GenZ Cafe verification code is ${otp}. It expires in ${OTP_TTL_MINUTES} minutes.`,sender,otp,otp_expiry:String(OTP_TTL_MINUTES),otp_length:'6'});
  const response=await fetch(`https://api.msg91.com/api/sendotp.php?${params.toString()}`,{method:'GET',cache:'no-store'});
  if(!response.ok) throw new Error('OTP delivery failed');
  const result=await response.json() as {type?:string};
  if(result.type&&result.type!=='success') throw new Error('OTP delivery failed');
}
...
export async function requestCustomerOtp(rawMobile:string){
  ...
  const now=new Date();
  const recent=await pool.query<RowDataPacket[]>('SELECT id FROM customer_otp_challenges WHERE mobile=? AND created_at>? LIMIT 1',[mobile,new Date(Date.now()-30000)]);
  if(recent[0].length) throw new Error('Please wait before requesting another OTP');
  const hourly=await pool.query<RowDataPacket[]>('SELECT COUNT(*) AS count FROM customer_otp_challenges WHERE mobile=? AND created_at>?',[mobile,new Date(Date.now()-3600000)]);
  if(Number((hourly[0][0] as {count:number}).count)>=5) throw new Error('Too many OTP requests. Try again later');
  ...
  await transaction(async(connection:PoolConnection)=>{
    await connection.execute('UPDATE customer_otp_challenges SET consumed_at=? WHERE mobile=? AND consumed_at IS NULL',[now,mobile]);
    await connection.execute('INSERT INTO customer_otp_challenges(id,mobile,otp_hash,expires_at,attempts,created_at) VALUES (?,?,?,?,?,?)',[challengeId,mobile,hash(otp),new Date(Date.now()+OTP_TTL_MINUTES*60000),0,now]);
  });
  try { await sendOtpSms(mobile,otp); }
  catch(error){ await pool.execute('UPDATE customer_otp_challenges SET consumed_at=? WHERE id=?',[new Date(),challengeId]); throw error; }
  return process.env.NODE_ENV!=='production'&&process.env.GENZ_OTP_DEV_MODE==='true'?{challengeId,devOtp:otp}:{challengeId};
}

export async function verifyCustomerOtp(challengeId:string,rawMobile:string,otp:string){
  const mobile=normalizeMobile(rawMobile);
  if(!/^OTP-[a-f0-9-]{30,}$/.test(challengeId)||!/^\d{6}$/.test(otp)||!mobile) throw new Error('Invalid verification request');
