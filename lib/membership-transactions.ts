...
export async function createMembership(input:{id:string;name:string;mobile:string;governmentIdType:GovernmentIdType;governmentIdNumber:string;amount:number;method:MembershipPaymentMethod;staffId:string}){
 return transaction(async c=>{
  await assertDailyCloseOpen(c);
  const memberId=input.id.trim();
  const amount=money(Number(input.amount));
  if(!/^GENZFAM\d{3}$/.test(memberId)||!input.name.trim()||!/^[0-9+() -]{7,20}$/.test(input.mobile.trim()))throw Error('INVALID_MEMBER_DETAILS');
  ...
  await c.execute('INSERT INTO members(id,name,mobile,government_id_type,government_id_number,expires_at,active,created_at,updated_at) VALUES(?,?,?,?,?,NULL,TRUE,NOW(3),NOW(3))',[memberId,input.name.trim(),input.mobile.trim(),input.governmentIdType,input.governmentIdNumber.trim()]);
  if(amount>0){
   const txId=id('MTX');
   await c.execute('INSERT INTO membership_transactions(id,member_id,type,amount,method,status,previous_expires_at,new_expires_at,created_by,created_at) VALUES(?,?,?,?,?,\'CAPTURED\',?,?,?,NOW(3))',[txId,memberId,'NEW',amount,input.method,null,null,input.staffId]);
...
export async function renewMembership(input:{memberId:string;newExpiresAt:string;amount:number;method:MembershipPaymentMethod;staffId:string}){
 return transaction(async c=>{
  ...
  const expiresAt=/^\d{4}-\d{2}-\d{2}$/.test(input.newExpiresAt)?input.newExpiresAt:'';
  const amount=money(Number(input.amount));
  ...
  const previous=rows[0].expires_at?String(rows[0].expires_at).slice(0,10):null;
  const txId=id('MTX');
...
export async function listMembershipTransactions(memberId:string){
 ...
 return rows.map(r=>({id:String(r.id),type:String(r.type),amount:Number(r.amount),method:String(r.method),status:String(r.status),previousExpiresAt:r.previous_expires_at?String(r.previous_expires_at).slice(0,10):null,newExpiresAt:r.new_expires_at?String(r.new_expires_at).slice(0,10):null,createdAt:new Date(r.created_at).toISOString()}));
}
