...
import {listSessionRefunds} from './session-refunds';
...
export type SessionReceipt={
  ...
  totals:{gaming:number;food:number;depositApplied:number;groupAllocatedGaming:number;groupAllocatedFood:number;adjustments:number;creditApplied:number;paid:number;refunded:number;outstanding:number};
  payments:Array<{id:string;method:string;amount:number;status:string;createdAt:string;voidedAt?:string;voidReason?:string}>;
  refunds:Array<{id:string;settlementId:string;amount:number;method:string;status:string;reference?:string;reason:string;createdAt:string}>;
  currency:'INR';
};
...
export async function getSessionReceipt(sessionId:string):Promise<SessionReceipt>{
  ...
  const [billing,settlement,payments,refunds]=await Promise.all([calculateSessionBilling(id),getSessionSettlement(id),listSessionPayments(id),listSessionRefunds(id)]);
