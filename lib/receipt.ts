...
export type SessionReceipt={
  ...
  payments:Array<{id:string;method:string;amount:number;status:string;createdAt:string;voidedAt?:string;voidReason?:string}>;
  currency:'INR';
};
...
export async function getSessionReceipt(sessionId:string):Promise<SessionReceipt>{
  ...
  return transaction(async(c:PoolConnection)=>{
    const [sessionRows]=await c.query<any[]>('SELECT id,station_id,customer_name,status FROM sessions WHERE id=? LIMIT 1',[id]);
    if(!sessionRows[0])throw Error('SESSION_NOT_FOUND');
    const session=sessionRows[0];
    const billing=await calculateSessionBilling(id,c);
    const settlement=await getSessionSettlement(id);
    const [rows]=await c.query<ReceiptOrderRow[]>(
      `SELECT o.id,o.status,o.payment_mode,o.payment_status,o.created_at,o.total,
              oi.name AS item_name,oi.qty,oi.unit_price,sp.display_name AS participant_name
         FROM orders o
         LEFT JOIN order_items oi ON oi.order_id=o.id
         LEFT JOIN session_participants sp ON sp.id=oi.participant_id
        WHERE o.session_id=? AND o.status<>'CANCELLED'
        ORDER BY o.created_at ASC,o.id ASC,oi.id ASC`,[id]);
    const byOrder=new Map<string,SessionReceipt['food'][number]>();
    for(const row of rows){
      let order=byOrder.get(String(row.id));
      if(!order){order={orderId:String(row.id),status:String(row.status),paymentMode:String(row.payment_mode),paymentStatus:String(row.payment_status),createdAt:new Date(row.created_at).toISOString(),total:money(row.total),items:[]};byOrder.set(order.orderId,order)}
      if(row.item_name)order.items.push({name:String(row.item_name),qty:Number(row.qty)||0,unitPrice:money(row.unit_price),participantName:row.participant_name?String(row.participant_name):undefined});
    }
    ...
    return {
      ...
      gaming:{total:billing.gamingTotal,participants:billing.participants.map(p=>({name:p.displayName,charge:p.charge,memberTier:p.memberTier}))},
      food:Array.from(byOrder.values()),
      totals:{gaming:billing.gamingTotal,food:settlement.foodTotal,depositApplied:settlement.depositApplied,groupAllocatedGaming:settlement.groupAllocatedGaming,groupAllocatedFood:settlement.groupAllocatedFood,adjustments:settlement.adjustments,creditApplied:settlement.creditApplied,paid:settlement.paid,outstanding:settlement.outstanding},
      payments:payments.map(p=>({id:p.id,method:p.method,amount:p.amount,status:p.status,createdAt:p.createdAt,voidedAt:p.voidedAt,voidReason:p.voidReason})),
      currency:'INR',
    };
  });
}
