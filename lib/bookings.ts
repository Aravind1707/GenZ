...
type BookingRow=RowDataPacket & {id:string;station_id:string;customer_name:string;member_id:string|null;starts_at:Date;ends_at:Date;status:BookingStatus;deposit:number;notes:string|null};
const map=(b:BookingRow):Booking=>({id:b.id,stationId:b.station_id,customerName:b.customer_name,memberId:b.member_id||undefined,startsAt:new Date(b.starts_at).toISOString(),endsAt:new Date(b.ends_at).toISOString(),status:b.status,deposit:Number(b.deposit),notes:b.notes||undefined});
...
export async function createBooking(input:Omit<Booking,'id'|'status'>&{status?:BookingStatus}){
